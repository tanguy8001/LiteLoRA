...
def get_backbone(args, pretrained=False):
    ...
    ## Lora version
    if name == "vit_base_patch16_224_litelora":
        from backbone.lora_gumbel import LoRA_ViT_timm as LoRA_ViT_timm_gumbel
        model = timm.create_model("vit_base_patch16_224", pretrained=True, num_classes=0)
        model = LoRA_ViT_timm_gumbel(vit_model=model.eval(), r=10, num_classes=10, increment=args['increment'], filepath=args['filepath'], args=args)
        model.out_dim = 768
        return model
