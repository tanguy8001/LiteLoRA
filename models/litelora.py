...
from utils.gumbel_utils import sparsity_loss

import timm
from backbone.lora_gumbel import LoRA_ViT_timm
...
class Learner(BaseLearner):
    ...
    def update_network(self, index=True):
        ...
        cur_task_index=self._cur_task, args=self.args)
        model.out_dim = 768
        return model
    ...
    def _update_representation(self, train_loader, test_loader, optimizer, scheduler):
        ...
        optimizer_p1 = optim.SGD([
            ...
            {'params': alpha_params, 'lr': self.args["alpha_lr"], 'weight_decay': self.args["weight_decay"]},
        ...
        self._train_phase(train_loader, test_loader, optimizer_p1, scheduler_p1,
        ...
        gumbel_gate.unfreeze_logits([self._cur_task])
        if self._cur_task > 0:
            other_indices = list(range(self._cur_task))
            gumbel_gate.freeze_logits(other_indices)

        gumbel_gate.phase = 2

        # Use small batch size for Phase 2 to get more gradient steps on the single logit
        phase2_loader = DataLoader(train_loader.dataset, batch_size=self.args["batch_size_p2"], shuffle=True, num_workers=num_workers)

        logit_before = gumbel_gate.gate_logits[self._cur_task].item()
        self._train_phase(phase2_loader, test_loader, optimizer, scheduler,
                         self.args["lambda_sparsity"], phase=2, epochs=selection_epochs)
        logit_after = gumbel_gate.gate_logits[self._cur_task].item()
        
        logging.info(f'[Phase 2] Logit {self._cur_task}: {logit_before:.4f} -> {logit_after:.4f} (delta={logit_after-logit_before:.4f})')
    ...
    def _train_phase(self, train_loader, test_loader, optimizer, scheduler,
        ...
        trainable_names = [n for n, p in self._network.named_parameters() if p.requires_grad]
        logging.info(f"Trainable params at start of Phase {phase}: {trainable_names[:10]}... (Total: {len(trainable_names)})")

        for epoch in prog_bar:
            self._network.train()
            losses, correct, total, steps = 0.0, 0, 0, 0
            for _, inputs, targets in train_loader:
                ...
                loss_reg = 0
                if phase == 2:
                    mode = 'l1' if getattr(gumbel_gate, 'binary_beta', False) else 'entropy'
                    num_reg_blocks = 0
                    for blk in blocks:
                        qkv_layer = blk.attn.qkv
                        if hasattr(qkv_layer, 'last_beta_q') and qkv_layer.last_beta_q is not None:
                            # Only regularize current task's beta (last element);
                            # previous tasks' logits are frozen so their betas are constants
                            loss_reg += sparsity_loss(qkv_layer.last_beta_q[-1:], mode=mode)
                            loss_reg += sparsity_loss(qkv_layer.last_beta_v[-1:], mode=mode)
                            num_reg_blocks += 1
                    # Normalize by number of blocks — the single shared logit
                    # accumulates gradients from all blocks, making lambda 24x too sensitive
                    if num_reg_blocks > 0:
                        loss_reg = loss_reg / (2 * num_reg_blocks)
                loss = loss_clf + lambda_sparsity * loss_reg
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                losses += loss.item()
                ...
                total += len(targets)
            if epoch == 0:
                logging.info(f"Phase {phase}: {steps} steps/epoch, {total} samples, batch_size={inputs.shape[0]}")
            scheduler.step()
            train_acc = np.around(tensor2numpy(correct) * 100 / total, decimals=2)
            if epoch % 5 == 0:
                test_acc = self._compute_accuracy(self._network, test_loader)
                info = f"Phase {phase}, Epoch {epoch+1}/{epochs} => Loss {losses/len(train_loader):.3f}, Train {train_acc:.2f}%, Test {test_acc:.2f}%"
            else:
                info = f"Phase {phase}, Epoch {epoch+1}/{epochs} => Loss {losses/len(train_loader):.3f}, Train {train_acc:.2f}%"
            prog_bar.set_description(info)
        logging.info(info)
    ...
    def _conditional_growth_decision(self):
        ...
        with torch.no_grad():
            betas = gumbel_gate.get_betas(task_indices, tau=1.0)
        current_beta = betas[-1].item()
        logging.info(f"[Conditional Growth] Task {current_task} | current beta: {current_beta:.3f}")
        logging.info(f"{'Task':<6} {'alpha':<8} {'logit':<8} {'beta':<10} {'Decision':<10}")
        for idx, task_idx in enumerate(task_indices):
            ...
            if task_idx == current_task:
                threshold = 0.5 if getattr(gumbel_gate, "binary_beta", False) else 0.05
                decision = "PRUNE" if current_beta < threshold else "KEEP"
        ...
        if current_beta < threshold:
            ...
            logging.info(f"Task {current_task} PRUNED: beta={current_beta:.2f} < {threshold}")
        else:
            gumbel_gate.keep_task(current_task)
            logging.info(f"Task {current_task} KEPT: beta={current_beta:.2f} >= {threshold}")
